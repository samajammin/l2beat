...
describe(PriceRepository.name, () => {
  ...
  describe(PriceRepository.prototype.getLatestKnownDateByToken.name, () => {
    it('saved token', async () => {
      const result = await repository.getLatestKnownDateByToken(CoingeckoId('uniswap'))
  
      expect(result).toEqual(START.add(-1, 'hours'))
    })

    it('not saved token', async () => {
      const result = await repository.getLatestKnownDateByToken(CoingeckoId('unknown-token'))
